...
WORKDIR /home/application
RUN /usr/lib/graalvm/bin/native-image --no-server -cp build/libs/kuanto-renta-*.jar
RUN chmod 755 bootstrap
RUN chmod 755 kuantorenta
RUN zip -j function.zip bootstrap kuantorenta
EXPOSE 8080
ENTRYPOINT ["/home/application/kuantorenta"]
